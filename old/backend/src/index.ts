...
const dbToken = process.env.DATABASE_AUTH_TOKEN || (isProd ? process.env.DATABASE_AUTH_TOKEN_PROD : process.env.DATABASE_AUTH_TOKEN_DEV);
...
(async () => {
  ...
  if (latestWeek) {
    ...
    const nextWeek = await prisma.week.findFirst({ where: { startDate: nextMonday } });
