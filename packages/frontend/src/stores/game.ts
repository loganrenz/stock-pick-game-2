  ...
  actions: {
    ...
    async submitPick(symbol: string) {
      ...
      try {
        ...
        await api.post('/picks', { symbol, weekId: this.currentWeek.id });
    ...
    async submitNextWeekPick(symbol: string, weekId: number, user: any) {
      ...
      try {
        ...
        if (userPick) {
          ...
          await api.put('/picks/update', { pickId: userPick.id, symbol, entryPrice: 0 });
        } else {
          // Create new pick
          await api.post('/picks/create', { symbol, weekId, entryPrice: 0 });
        }
        await this.fetchWeeks();
        await this.fetchCurrentWeek();
      } catch (err) {
    ...
    async createPick(pickData: CreatePickData) {
      try {
        const response = await api.post('/picks/create', pickData);
        const newPick = {
          ...response.data,
    ...
    async updatePick(pickId: number, updateData: Partial<Pick>) {
      try {
        const response = await api.put('/picks/update', {
          pickId,
          ...updateData,
        });
